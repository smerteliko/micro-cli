...
#[AsConsoleCommand(name: 'setup', description: 'Interactive project setup wizard')]
class SetupCommand extends Command
{
	public function execute(InputInterface $input, OutputInterface $output): int
	{
		...
		$environment = $io->choice(
			'Select the environment for this project:',
		...
		$modules = $io->choice(
			...
			['Redis Cache', 'MySQL Database', 'ElasticSearch', 'RabbitMQ'],
			'1,2', // Дефолтные значения через запятую
			true   // Включаем multiSelect!
		);

		$io->text("Installing modules: <info>" . implode(', ', $modules) . "</info>");
